...
export function parseDsRequest(raw: unknown): DsRequestPayload {
  ...
  return result.data as DsRequestPayload;
}
...
function assertNoServerOnlyQueryFields(query: Record<string, unknown>): void {
...
export function validateQuery(query: unknown): void {
  if (query != null && typeof query === 'object' && !Array.isArray(query)) {
    assertNoServerOnlyQueryFields(query as Record<string, unknown>);
  }
