...
export function canBePrimaryKey(pgType: string, maxLength?: number) {
...
export function getAttributeType(pgType: string, maxLength: number) {
...
export function getAttributes(columns: Column[], index?: number) {
...
function getDataType(pgType: string, maxLength: number, allowDecimals: boolean, excludeTime: boolean) {
  const { base, isArray } = normalizePgTypeBase(pgType);
  if (isArray) {
    if (base === 'character varying' || base === 'character' || base === 'text') {
      return 'string[]';
...
export function getInterfaceFields(columns: Column[], index?: number) {
  ...
  const fields = columns
    .map((column) => {
      ...
      if (type === 'unknown') {
        return `  ${camelCase(column.name)}${index ?? ''}${
          column.nullable ? '?' : ''
        }: ${type}${column.nullable ? ' | null' : ''}; // ${column.type}`;
      }
      return `  ${camelCase(column.name)}${index ?? ''}${
        column.nullable ? '?' : ''
...
export function getDefaultOperatorForType(type: string) {
