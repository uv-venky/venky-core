...
function allowsDecimals(pgType: string) {
...
function includesTime(pgType: string) {
  const { base } = normalizePgTypeBase(pgType);
  return ['timestamp with time zone', 'timestamp without time zone'].includes(base);
}

export const genColumns = withDBSessionAction(
  async (client, _session, tableName: string, schemaName: string): Promise<Column[]> => {
...
export const getTableNames = withDBSessionAction(async (client, _session, filter: string): Promise<Table[]> => {
...
`;
  const params = [`${filter}%`];
  const res = await client.query<Table>(query, params);
  return res.rows;
});
