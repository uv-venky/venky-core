...
export default ({ dsName, moduleCode, columns, columnOrder, index, template }: State) => {
  let cols = columns
  ...
  cols =
    template === 'drill-down' && moduleCode === 'amazon'
      ? cols.filter(
          (c) => !['state', 'region', 'site_name', 'counter_date', 'customer_name', 'customer_number'].includes(c.name),
        )
      : cols;
  const fields = cols
    .map((column) => {
