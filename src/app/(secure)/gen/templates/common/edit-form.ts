...
export default ({ dsName, moduleCode, columns, columnOrder, index }: State): string => {
  let hasYN = false;
  const hasTF = false;
...
import { ${hasBoolean ? 'BooleanInput, ' : ''} ${
    hasDate ? 'DateInputField, ' : ''
  } ${hasTime ? 'TimeInputField, ' : ''} ${hasNumber ? 'NumberInput, ' : ''} ${hasYN ? 'YNInput, ' : ''} ${
    hasTF ? 'TFInput, ' : ''
  } ${hasText ? 'TextInput, ' : ''} } ${'from'} '@/components/core/page/fields';
