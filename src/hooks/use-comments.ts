...
import type { CommentChannel, ChannelPayload } from '@/lib/sse/types';

export function useComments(context: string, contextId: string) {
  ...
  const fetchComments = useCallback(async () => {
    ...
    if (!hasMoreRef.current && currentCursor !== null) return;
  ...
  // Create comment
  const createComment = async (comment: NewComment) => {
  ...
  const reactToComment = async (commentId: string, emoji: string | null): Promise<void> => {
  ...
  const commentChannel = useMemo<CommentChannel>(() => `comment:${context}:${contextId}`, [context, contextId]);

  // Handle SSE messages
  const handleSSEMessage = useCallback(
    (_channel: CommentChannel, data: ChannelPayload<CommentChannel>) => {
