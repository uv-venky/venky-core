 ...
 */
export function getCredentials(source: CredentialSource, userEnvVars?: EnvVarConfig): EnvVarConfig {
...
export function enrichStepInput(
  ...
  const actionHandlers: Record<string, () => void> = {
    ...
    'Send Slack Message': () => enrichSlackCredentials(enrichedInput, credentials),
...
function enrichLinearCredentials(input: Record<string, unknown>, credentials: EnvVarConfig): void {
...
function enrichResendCredentials(input: Record<string, unknown>, credentials: EnvVarConfig): void {
...
function enrichSlackCredentials(input: Record<string, unknown>, credentials: EnvVarConfig): void {
  if (credentials.SLACK_API_KEY) {
    input.apiKey = credentials.SLACK_API_KEY;
  }
}

function enrichAICredentials(input: Record<string, unknown>, credentials: EnvVarConfig): void {
  if (credentials.AI_GATEWAY_API_KEY) {
    input.apiKey = credentials.AI_GATEWAY_API_KEY;
  }
}

function enrichDatabaseCredentials(input: Record<string, unknown>, credentials: EnvVarConfig): void {
  if (credentials.DATABASE_URL) {
    input.databaseUrl = credentials.DATABASE_URL;
  }
}

function enrichFirecrawlCredentials(input: Record<string, unknown>, credentials: EnvVarConfig): void {
