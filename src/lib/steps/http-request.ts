...
function parseBody(httpMethod: string, httpBody?: string): string | undefined {
  ...
  try {
    ...
    return Object.keys(parsedBody).length > 0 ? JSON.stringify(parsedBody) : undefined;
  } catch {
 ...
 */
export async function httpRequestStep(input: HttpRequestInput): Promise<HttpRequestResult> {
