...
import type { Channel, ChannelPayload, SSEEventType, SSEMessage } from '../types';
 ...
 * ```
 */
export async function publishSSE<T extends Channel>(
  client: PgPoolClient,
  channel: T,
  data: ChannelPayload<T>,
): Promise<void> {
  const eventType: SSEEventType = 'sse';
