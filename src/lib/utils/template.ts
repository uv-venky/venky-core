...
function processNewFormatReference(trimmed: string, nodeOutputs: NodeOutputs, match: string): string {
...
function processLegacyDollarReference(trimmed: string, nodeOutputs: NodeOutputs, match: string): string {
...
function processLegacyLabelReference(trimmed: string, nodeOutputs: NodeOutputs, match: string): string {
 ...
 */
function findNodeOutputByLabel(label: string, nodeOutputs: NodeOutputs): { label: string; data: unknown } | undefined {
...
export function formatTemplateForDisplay(template: string): string {
  ...
  return template.replace(/\{\{@[^:]+:([^}]+)\}\}/g, (_match, rest) => `{{${rest}}}`);
}
