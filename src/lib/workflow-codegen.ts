...
export function generateWorkflowCode(
  ...
  function generateEmailActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
  ...
  function generateTicketActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
  ...
  function generateDatabaseActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
    ...
    const lines = [`${indent}const ${varName} = await ${stepInfo.functionName}({`];
  ...
  function generateHTTPActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
  ...
  function generateAiTextActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
  ...
  function generateAiImageActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
    ...
    const imageModel = (node.data.config?.imageModel as string) || 'google/imagen-4.0-generate';
  ...
  function generateSlackActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
  ...
  function generateFirecrawlActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
  ...
  function generateV0CreateChatActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
  ...
  function generateV0SendMessageActionCode(node: WorkflowNode, indent: string, varName: string): string[] {
   ...
   */
  function formatFieldValue(fieldType: string, value: unknown, indent: string, key: string): string {
    const fieldTypeFormatters: Record<string, () => string> = {
      ...
      'action-params': () => `${indent}  ${key}: ${formatTemplateValue(String(value))},`,
      'single-row-update': () => `${indent}  ${key}: ${formatTemplateValue(String(value))},`,
      'match-builder': () => `${indent}  ${key}: ${formatTemplateValue(String(value))},`,
      'multi-select': () => `${indent}  ${key}: ${formatTemplateValue(String(value))},`,
  ...
  function generateActionNodeCode(node: WorkflowNode, nodeId: string, indent: string, varName: string): string[] {
    const actionType = node.data.config?.actionType as string;
  ...
  function generateConditionNodeCode(node: WorkflowNode, nodeId: string, indent: string): string[] {
  ...
  function processTriggerNode(nodeId: string, indent: string): { lines: string[]; wasSkipped: boolean } {
    const nextNodes = edgesBySource.get(nodeId) || [];
  ...
  function processActionNode(node: WorkflowNode, nodeId: string, varName: string, indent: string): string[] {
    const lines: string[] = [];
    const actionType = node.data.config?.actionType as string;
   ...
   */
  function generateBranchCode(nodeId: string, indent: string, branchVisited: Set<string>): string[] {
   ...
   */
  function generateActionCallCode(node: WorkflowNode, indent: string): string[] {
    const actionType = node.data.config?.actionType as string;
    const actionLabel = node.data.label || actionType || 'Unknown Action';
    const stepInfo = getStepInfo(actionType);
   ...
   */
  function generateChildrenCode(childIds: string[], indent: string, branchVisited: Set<string>): string[] {
   ...
   */
  function generateAsyncIIFEBranch(nodeId: string, indent: string, isLast: boolean): string[] {
    ...
    return [`${indent}  (async () => {`, ...branchCode, `${indent}  })()${comma}`];
  }

  /**
   * Generate code for multiple nodes from trigger
   */
  function generateParallelNodeCode(nodeIds: string[], indent: string): string[] {
    if (nodeIds.length === 0) {
      return [];
    }
   ...
   */
  function buildPluginConfigParams(config: Record<string, unknown>, actionType: string, indent: string): string[] {
    const action = findActionById(actionType);
    if (!action) {
      return [];
    }
   ...
   */
  function buildFallbackConfigParams(config: Record<string, unknown>, indent: string): string[] {
    const params: string[] = [];
   ...
   */
  function buildActionConfigParams(node: WorkflowNode, indent: string): string[] {
    const actionType = node.data.config?.actionType as string;
    const config = node.data.config || {};
  ...
  function processNextNodes(nodeId: string, currentLines: string[], indent: string): string[] {
    const nextNodes = edgesBySource.get(nodeId) || [];
