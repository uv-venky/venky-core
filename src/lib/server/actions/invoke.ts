 ...
 */
function assertValidActionParams(action: string, args: unknown[]): void {
  const { ACTION_PARAM_SCHEMAS: registrySchemas } = getActionRegistry();
  const entries = registrySchemas?.[action] ?? GENERATED_PARAM_SCHEMAS[action as keyof typeof GENERATED_PARAM_SCHEMAS];
  if (!entries) return;
...
export async function invokePublicAction<T extends ActionName>(
  client: PgPoolClient,
  action: T,
  ...args: ActionParams<T>
) {
