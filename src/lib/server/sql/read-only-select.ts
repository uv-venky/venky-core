...
function assertNoDeniedFunctions(node: unknown): void {
  if (!node || typeof node !== 'object') return;
  if (Array.isArray(node)) {
    for (const child of node) assertNoDeniedFunctions(child);
    return;
  }
  const obj = node as Record<string, unknown>;
  if (obj.type === 'function' || obj.type === 'aggr_func') {
    const rawName = obj.name as unknown;
    let fnName: string | undefined;
    if (typeof rawName === 'string') {
      fnName = rawName;
    } else if (rawName && typeof rawName === 'object') {
      const inner = (rawName as { name?: unknown }).name;
      if (typeof inner === 'string') {
        fnName = inner;
      } else if (Array.isArray(inner)) {
        const last = inner[inner.length - 1] as { value?: unknown } | undefined;
        if (last && typeof last.value === 'string') fnName = last.value;
      }
    }
...
export function assertReadOnlySelect(sql: string): void {
  ...
  let ast: ReturnType<typeof sqlParser.astify>;
  try {
    ast = sqlParser.astify(cleaned, { database: 'postgresql' });
  } catch (err) {
    const e = err as Error & { location?: { start?: { line?: number; column?: number } } };
...
export function getPgTypeName(field: FieldDef): string {
