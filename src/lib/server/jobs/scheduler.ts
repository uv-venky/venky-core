import { getAllJobs, type JobEntry } from './registry';
...
interface ExecuteJobRunParams {
  client: PgPoolClient;
  job: JobEntry;
  appId: string;
  node: string;
  logContext: 'Scheduled' | 'On-demand';
  trackHeartbeat?: boolean;
}

interface ExecuteJobRunResult {
  success: boolean;
  jobRunId: number;
  error: string | null;
}
...
}: ExecuteJobRunParams): Promise<ExecuteJobRunResult> {
  const { rows: runRows } = await client.query<{ job_run_id: number }>(
...
async function scheduleJob(job: JobEntry): Promise<void> {
  ...
  const scheduleNext = async () => {
    await transaction(async (client) => {
      ...
      const res = await client.query<{ next_run: Date }>(
  ...
  const runJob = async () => {
    const nextRun = await withAdvisoryLock(lockKey, async (client) => {
      ...
      const { appId, schedulerId } = config;
      const { rows } = await client.query<{ next_run: Date }>(
...
export async function runJobByName(jobName: string, options?: RunJobByNameOptions): Promise<RunJobByNameResult> {
  ...
  return withBlockingAdvisoryLock(hashJobName(jobName), async (client: PgPoolClient) => {
