...
export async function sendQueuedEmails(): Promise<void> {
  ...
  await transaction(async (client) => {
    ...
    const { rows } = await client.query<{
      ...
      `SELECT request_id, mail_options, attempt_count
        ...
        await client.query(
          `UPDATE ${PREFIX}email_requests
              SET sent_at = statement_timestamp(),
                  attempt_count = $2,
                  last_error = NULL
            WHERE request_id = $1 AND app_id = $3`,
          [row.request_id, row.attempt_count + 1, appId],
        );
      } catch (e) {
        ...
        if (attempts >= MAX_RETRIES) {
          await client.query(
            `UPDATE ${PREFIX}email_requests
              ...
              WHERE request_id = $1 AND app_id = $4`,
            [row.request_id, attempts, msg, appId],
          );
        } else {
          await client.query(
            `UPDATE ${PREFIX}email_requests
