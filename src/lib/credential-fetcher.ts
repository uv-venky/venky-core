...
const SYSTEM_CREDENTIAL_MAPPERS: Record<string, (config: IntegrationConfig) => WorkflowCredentials> = {
  database: (config) => {
 ...
 */
function mapIntegrationConfig(integrationType: IntegrationType, config: IntegrationConfig): WorkflowCredentials {
 ...
 */
export async function fetchCredentials(integrationId: string): Promise<WorkflowCredentials> {
  ...
  if (!integration) {
    if (logger.debugEnabled) {
      logger.debug('[Credential Fetcher] Integration not found');
    }
    return {};
  }

  if (logger.debugEnabled) {
    logger.debug('[Credential Fetcher] Found integration:', integration.type);
  }

  const credentials = mapIntegrationConfig(integration.type, integration.config);

  if (logger.debugEnabled) {
    logger.debug('[Credential Fetcher] Returning credentials for type:', integration.type);
  }
 ...
 */
export function fetchIntegrationCredentials(integrationId: string): Promise<WorkflowCredentials> {
