...
export class DbRateLimiter {
  constructor(private readonly opts: DbRateLimitOptions) {}
   ...
   */
  async isRateLimited(key: string | null): Promise<boolean> {
    if (!key) return true;
    ...
    try {
      ...
      const { rows } = await pool.query<{ request_count: number }>(
        `INSERT INTO uv_rate_limit_buckets (bucket_key, window_start, request_count, updated_at)
         VALUES ($1, $2, 1, NOW())
...
function getOrCreateLimiter(key: string, opts: DbRateLimitOptions): DbRateLimiter {
...
export function getPasswordResetDbLimiter(): DbRateLimiter {
 ...
 */
export function getSignInUserDbLimiter(): DbRateLimiter {
 ...
 */
export function getSignInIpDbLimiter(): DbRateLimiter {
 ...
 */
export function getMagicLinkDbLimiter(): DbRateLimiter {
 ...
 */
export function getRefreshTokenDbLimiter(): DbRateLimiter {
 ...
 * guessing. 30 / minute.
 */
export function getSsoCallbackDbLimiter(): DbRateLimiter {
 ...
 */
export async function cleanupDbRateLimitBuckets(): Promise<number> {
  try {
    const pool = getPool();
    const { rowCount } = await pool.query(
      `DELETE FROM uv_rate_limit_buckets WHERE updated_at < NOW() - INTERVAL '24 hours'`,
    );
    return rowCount ?? 0;
  } catch (err) {
