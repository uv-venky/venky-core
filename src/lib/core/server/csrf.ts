...
function readNamedCookieFromHeader(cookieHeader: string, name: string): string | null {
...
async function setResponseCsrfCookie(token: string, session: Session): Promise<void> {
 ...
 */
export async function assertCsrf(client: PgPoolClient, session: Session, req: Request): Promise<void> {
  ...
  const result = await client.query<{ csrf_token: string | null }>(
    `SELECT csrf_token FROM ${PREFIX}user_sessions WHERE session_id = $1 LIMIT 1`,
    [session.id],
  );
  ...
  if (!stored) {
    ...
    const upd = await client.query<{ csrf_token: string }>(
    ...
    if (upd.rows[0]?.csrf_token) {
      ...
      const reread = await client.query<{ csrf_token: string | null }>(
        `SELECT csrf_token FROM ${PREFIX}user_sessions WHERE session_id = $1 LIMIT 1`,
        [session.id],
      );
