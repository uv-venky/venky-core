...
export async function bulkCreateUsers(
  ...
  const emailUsers: { email: string; userName: string; password: string }[] = [];
  let i = 1;
  ...
  try {
    ...
    return rows.map((r: { user_name: string }) => r.user_name);
  } catch (e) {
...
export async function bulkAssignRolesToUsers(
  client: PgPoolClient,
  data: {
    createdBy: string;
    assignments: { userName: string; roles: string[]; startDate?: Date; endDate?: Date }[];
  },
): Promise<void> {
  ...
  const valueClauses: string[] = [];
  const params: (string | Date | undefined)[] = [];
  let i = 1;
...
export async function endDateUser(
  client: PgPoolClient,
  data: {
    updatedBy: string;
    userName: string;
  ...
  try {
    await client.query(
      `UPDATE ${PREFIX}users SET end_date = $1, updated_at = $2, updated_by = $3 WHERE user_name = $4`,
      [endDate, now, updatedBy, userName],
    );
  } catch (e) {
...
export async function reactivateUser(client: PgPoolClient, data: { updatedBy: string; userName: string }) {
  const now = new Date().toISOString();
  const { userName, updatedBy } = data;

  try {
    await client.query(
      `UPDATE ${PREFIX}users SET end_date = NULL, updated_at = $1, updated_by = $2 WHERE user_name = $3`,
      [now, updatedBy, userName],
    );
  } catch (e) {
...
export async function assignRolesToUser(
  ...
  const params = [] as (string | Date | undefined)[];
  let i = 1;
  const valueClauses = [] as string[];
...
export async function endDateRolesToUser(
  client: PgPoolClient,
  data: {
    updatedBy: string;
    roles: string[];
  ...
  try {
    await client.query(
      `UPDATE ${PREFIX}user_roles SET end_date = $1, updated_at = $2, updated_by = $3 WHERE user_name = $4 AND role_code IN (${placeholders})`,
      [endDate, now, updatedBy, userName, ...roles],
    );
  } catch (e) {
...
export async function syncUserRoles(
  ...
  }>(`SELECT role_code, end_date FROM ${PREFIX}user_roles WHERE user_name = $1 AND app_id = $2`, [userName, appId]);
