...
async function getAppsFromDatabase(currentAppId: string, userName: string): Promise<Team[]> {
  try {
    ...
    return result.rows.map((app) => ({
      name: app.name,
      menuTitle: app.name,
      logo: (app.icon || 'MiniLogo') as SidebarIcon,
...
export async function getUserTeams(session: Session): Promise<Team[]> {
  ...
  const clientTeams: Team[] = teams.map((team) => ({
    ...team,
 ...
 */
export async function checkPageAccess(session: Session, pathname: string): Promise<boolean> {
