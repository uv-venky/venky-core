...
import { isUserError, type ErrorResponse } from '@/lib/core/common/error';

const ADMIN_ROLES = ['admin', 'app_admin'] as const;

function isAdminSession(session?: Session | null): boolean {
  return session?.user?.roles?.some((role) => ADMIN_ROLES.includes(role as (typeof ADMIN_ROLES)[number])) ?? false;
}
 ...
 */
function buildTechnicalMessage(error: unknown): string {
 ...
 */
function getUserFriendlyDbMessage(error: DatabaseError): string | null {
 ...
 */
export function createErrorResponse(error: unknown, session?: Session | null): ErrorResponse {
