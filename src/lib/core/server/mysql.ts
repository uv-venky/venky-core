...
export function getPool() {
  ...
  let ssl: Record<string, string> | undefined;
...
export const transactionWithRetry = async <Output>(callback: (_client: mysql.PoolConnection) => Promise<Output>) => {
  let attempt = 1;
...
export const transaction = async <Output>(
  callback: (_client: mysql.PoolConnection) => Promise<Output>,
): Promise<[Output | null, Error | null]> => {
  const client = await newClient();
  try {
    await client.beginTransaction();
    const result = await callback(client);
    await client.commit();
    return [result as Output, null];
  } catch (e) {
    await client.rollback();
...
export async function resetTransaction(client: mysql.PoolConnection): Promise<void> {
  await client.rollback();
  await client.beginTransaction();
}
...
export async function withAdvisoryLock<T>(
  key: bigint,
  callback: (client: mysql.PoolConnection) => Promise<T>,
): Promise<T | null> {
  const client = await newClient();
  try {
    const [rows] = await client.query<mysql.RowDataPacket[]>('SELECT GET_LOCK(?, 0) AS locked', [key.toString()]);
