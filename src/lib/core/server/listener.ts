...
import { onServerEvent, type ServerEventName, type ServerEventPayload } from './server-events';
import type { Channel, SSEEventType, SSEMessage } from '@/lib/sse/types';
 ...
 */
export function getRetryTimeout(attempt: number): number {
...
export async function startListener() {
  ...
  try {
    ...
    client.on('notification', (msg) => {
      if (msg.payload) {
        try {
          const [eventName, payload] = JSON.parse(msg.payload) as [
            ServerEventName | SSEEventType,
            ServerEventPayload<ServerEventName> | SSEMessage<Channel>,
          ];

          // Handle SSE events by broadcasting to connected clients
          if (eventName === 'sse') {
            const ssePayload = payload as SSEMessage<Channel>;
            sseRegistry.broadcast(ssePayload.channel, ssePayload.data);
          } else {
            // Handle other server events
            onServerEvent(eventName as ServerEventName, payload as ServerEventPayload<ServerEventName>);
          }
        } catch (error) {
