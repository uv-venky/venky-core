 ...
 */
export function setRequestContextProvider(provider: RequestContextProvider) {
 ...
 */
export function getRequestContext(name: string): RequestContextProvider {
  const provider = globalThis._$venkyRequestContextProvider;
  if (!provider) {
    logger.error(`RequestContext provider not set! ${name}`);
    throw new Error(
      `RequestContext provider not set! ${name}. Call setRequestContextProvider() during server initialization.
