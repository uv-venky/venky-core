...
import { getMobileAuthSessionMetadata, type MobileAuthSessionData } from '@/lib/core/common/mobile-auth';

// Re-export types from common types for compatibility
export type { User, Session } from './lib/core/common/types/Auth';
import type { User, Session } from './lib/core/common/types/Auth';
...
const SESSION_TTL_MS =
  (process.env.SESSION_TTL_SECONDS ? Number.parseInt(process.env.SESSION_TTL_SECONDS, 10) : 12 * 60 * 60) * 1000;
...
async function isUserActive(user: DBUser) {
...
async function getUser(userName: string): Promise<DBUser | null> {
  const result = await executeQuery<DBUser>(
    `SELECT
      user_name,
      password_hash,
      email,
...
async function serverAuthorize(credentials: Partial<Record<'userName' | 'password', unknown>>) {
  ...
  if (parsedCredentials.success) {
    ...
    await executeQuery(
      `UPDATE ${PREFIX}users SET failed_login_attempts = failed_login_attempts + 1,
      last_failed_login = $1, last_failed_login_ip_address = $2 WHERE user_name = $3`,
      [new Date().toISOString(), ipAddress, userName],
    );
...
async function getUserFromDBUser(user: DBUser, sessionIndex?: string): Promise<User> {
  return {
    email: user.email,
...
function createMobileAuthSessionData(encryptedSessionId: string, expiresAt: Date, user: User): MobileAuthSessionData {
  return {
    encryptedSessionId,
...
export async function cacheAutoLoginSession(): Promise<void> {
  ...
  if (autoLoginUser) {
    if (process.env.NODE_ENV === 'production') {
      logger.warn(
        'AUTO_LOGIN_USER is set but NODE_ENV=production. Auto-login is disabled in production and will be ignored.',
      );
      return;
    }
    ...
    const autoLoginSession: Session = {
...
export async function auth(doNotSetSessionCookie = false): Promise<Session | null> {
  const cache = getCacheProvider();
  ...
  if (autoLoginUser && process.env.NODE_ENV !== 'production') {
    const cacheKey = `autoLoginSession.${autoLoginUser}`;
    let cachedAutoLoginSession = await cache.get<Session>(cacheKey);
    ...
    cachedAutoLoginSession = await cache.get<Session>(cacheKey);
    if (cachedAutoLoginSession) {
      return cachedAutoLoginSession;
    }
  ...
  let sessionIdUserName: string | null = null;
  try {
  ...
  const cachedSession = await cache.get<Session>(cacheKey);
  ...
  if (cachedSession) {
    const sessionResult = await executeQuery<{
      ...
      `SELECT session_id, user_name, expires_at, signed_out_at, csrf_token FROM ${PREFIX}user_sessions WHERE session_id = $1 AND app_id = $2`,
      [sessionId, appId],
    );
...
export async function signIn(
  ...
  if (token) {
    ...
    try {
      ...
      let sessionId = null;
      let cloudioRoles: string[] | undefined;
...
export async function refreshToken(encryptedSessionId: string): Promise<MobileAuthSessionData | null> {
  let sessionIdUserName: string | null = null;
  try {
    sessionIdUserName = await decrypt(encryptedSessionId);
  } catch (e) {
  ...
  const sessionResult = await executeQuery<{
    ...
    `SELECT session_id, user_name, user_id, ip_address, user_agent,
  ...
  await transaction(async (client) => {
    ...
    await client.query(
      ...
      [now, newSessionId, sessionId, appId],
    );
  });
 ...
 */
async function revokeSessionChain(startSessionId: string, appId: string): Promise<void> {
  const now = new Date();
  await transaction(async (client) => {
    let current: string | null = startSessionId;
    ...
    while (current && !seen.has(current)) {
      ...
      const result: { rows: Array<{ rotated_to_session_id: string | null }> } = await client.query(sql, [
        now,
        current,
        appId,
      ]);
...
async function setSessionCookie(encryptedSessionId: string, expiresAt: Date) {
  // Set session cookie
  try {
 ...
 * the session row.
 */
async function setCsrfCookie(csrfToken: string, expiresAt: Date) {
  try {
...
async function ensureBrowserSessionCsrf(params: {
  ...
  doNotSetSessionCookie: boolean;
  dbCsrfToken: string | null;
}): Promise<void> {
  ...
  if (!token) {
    ...
    const resolved = await transaction(async (con) => {
      const u = await con.query<{ csrf_token: string }>(
        `UPDATE ${PREFIX}user_sessions SET csrf_token = $1 WHERE session_id = $2 AND app_id = $3 AND csrf_token IS NULL RETURNING csrf_token`,
        [minted, sessionId, appId],
      );
      if (u.rows[0]?.csrf_token) {
        return u.rows[0].csrf_token;
      }
      const s = await con.query<{ csrf_token: string | null }>(
        `SELECT csrf_token FROM ${PREFIX}user_sessions WHERE session_id = $1 AND app_id = $2`,
        [sessionId, appId],
      );
  ...
  await transaction(async (con) => {
    ...
    await con.query(
      `INSERT INTO ${PREFIX}user_sessions_arch (
        ...
        user_name, user_id, session_id, ip_address, user_agent,
        csrf_token, expires_at, signed_in_at, last_accessed_at, signed_out_at, app_id, metadata
      FROM ${PREFIX}user_sessions WHERE session_id = $1 AND app_id = $2`,
      [session.id, appId],
    );
...
export async function clearSessionCache(userName: string): Promise<void> {
  const cache = getCacheProvider();
