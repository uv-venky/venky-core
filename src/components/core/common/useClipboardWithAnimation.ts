...
let _interpolate: typeof import('flubber').interpolate | null = null;
...
export function useClipboardWithAnimation(options: UseClipboardWithAnimationOptions) {
  const {
    paths,
    onSuccess,
    onError,
  ...
  const mixPathsRef = useRef<((t: number) => string) | null>(null);
  ...
  const togglePath = () => {
    ...
    if (!el || !mixPathsRef.current) return;
