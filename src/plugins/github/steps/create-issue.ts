...
async function stepHandler(input: CreateIssueCoreInput, credentials: GitHubCredentials): Promise<CreateIssueResult> {
  ...
  try {
    ...
    const response = await fetch(`${GITHUB_API_URL}/repos/${input.owner}/${input.repo}/issues`, {
      method: 'POST',
      headers: {
...
export async function createIssueStep(input: CreateIssueInput): Promise<CreateIssueResult> {
