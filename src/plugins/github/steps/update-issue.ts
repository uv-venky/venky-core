...
async function stepHandler(input: UpdateIssueCoreInput, credentials: GitHubCredentials): Promise<UpdateIssueResult> {
  ...
  try {
    ...
    const response = await fetch(`${GITHUB_API_URL}/repos/${input.owner}/${input.repo}/issues/${issueNum}`, {
      method: 'PATCH',
      headers: {
...
export async function updateIssueStep(input: UpdateIssueInput): Promise<UpdateIssueResult> {
