...
- `venky/` - Platform services (sleep, email, logging, activity, TTL store, short URLs, trigger workflow, call action). Internal plugin, no credentials. Use for built-in platform APIs.
- `datasource/` - DataSource CRUD (query, insert, update, delete). Internal plugin, no credentials.
