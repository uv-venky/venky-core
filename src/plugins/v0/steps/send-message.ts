...
type SendMessageResult = { success: true; chatId: string; demoUrl?: string } | { success: false; error: string };
 ...
 */
async function stepHandler(input: SendMessageCoreInput, credentials: V0Credentials): Promise<SendMessageResult> {
  const apiKey = credentials.V0_API_KEY;

  if (!apiKey) {
    return {
      success: false,
      error: 'V0_API_KEY is not configured. Please add it in Project Integrations.',
    };
  }
 ...
 */
export async function sendMessageStep(input: SendMessageInput): Promise<SendMessageResult> {
