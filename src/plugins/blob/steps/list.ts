...
async function stepHandler(input: ListBlobsCoreInput, credentials: BlobCredentials): Promise<ListBlobsResult> {
  const token = credentials.BLOB_READ_WRITE_TOKEN;

  if (!token) {
    return {
      success: false,
      error: 'BLOB_READ_WRITE_TOKEN is not configured. Please add it in Project Integrations.',
    };
  }
...
export async function listBlobsStep(input: ListBlobsInput): Promise<ListBlobsResult> {
