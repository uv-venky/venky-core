...
async function pollForResult(statusUrl: string, responseUrl: string, apiKey: string): Promise<FalImageToImageResponse> {
 ...
 */
async function stepHandler(input: FalImageToImageCoreInput, credentials: FalCredentials): Promise<ImageToImageResult> {
  ...
  try {
    ...
    if (queueResponse.status === 'IN_QUEUE' || queueResponse.status === 'IN_PROGRESS') {
      result = await pollForResult(queueResponse.status_url, queueResponse.response_url, apiKey);
    } else {
 ...
 */
export async function falImageToImageStep(input: FalImageToImageInput): Promise<ImageToImageResult> {
