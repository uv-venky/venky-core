 ...
 * Core logic
 */
async function stepHandler(input: SuperagentGuardCoreInput, credentials: SuperagentCredentials): Promise<GuardResult> {
  ...
  try {
    ...
    if (!content || typeof content !== 'object') {
      throw new Error('Invalid Guard API response: missing or invalid content structure');
    }

    const classification = content.classification;
    if (!classification || (classification !== 'pass' && classification !== 'block')) {
      throw new Error(
    ...
    return {
      ...
      reasoningText: choice?.message?.reasoningText,
    };
  } catch (error) {
 ...
 * Step entry point
 */
export async function superagentGuardStep(input: SuperagentGuardInput): Promise<GuardResult> {
