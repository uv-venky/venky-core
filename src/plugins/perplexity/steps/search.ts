...
async function stepHandler(
  ...
  try {
    const response = await fetch(PERPLEXITY_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a helpful search assistant. Provide concise, accurate answers with relevant sources.',
          },
          {
            role: 'user',
            content: input.query,
    ...
    const citations = (result.citations || []).map((c) => (typeof c === 'string' ? c : c.url));
...
function getSearchDomains(focus?: string): string[] | undefined {
  switch (focus) {
    case 'academic':
      return ['scholar.google.com', 'arxiv.org', 'pubmed.ncbi.nlm.nih.gov', 'researchgate.net'];
    case 'news':
      return ['reuters.com', 'apnews.com', 'bbc.com', 'nytimes.com', 'theguardian.com'];
    case 'youtube':
      return ['youtube.com'];
    case 'reddit':
      return ['reddit.com'];
 ...
 */
export async function perplexitySearchStep(input: PerplexitySearchInput): Promise<SearchResult> {
