...
type CreateTicketResult = { success: true; id: string; url: string; title: string } | { success: false; error: string };
 ...
 */
async function stepHandler(input: CreateTicketCoreInput, credentials: LinearCredentials): Promise<CreateTicketResult> {
  ...
  try {
    ...
    if (!targetTeamId) {
      const teamsResult = await linearQuery<TeamsQueryResponse>(apiKey, `query { teams { nodes { id name } } }`);
 ...
 */
export async function createTicketStep(input: CreateTicketInput): Promise<CreateTicketResult> {
