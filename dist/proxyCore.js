...
import { getRateLimiters } from './lib/core/server/ratelimit';
...
const NEXT_STATIC_PREFIXES = ['/_next/image'];
/**
 ...
 */
function handleStaticRequest({ pathname, requestUrl, validIp, nonce, unauthenticatedStaticRateLimiter, pathnamePrefixes, cspPolicy = getContentSecurityPolicy, }) {
...
function isExtensionBearingPath(pathname) {
    const lastSegment = pathname.split('/').pop() ?? '';
    return /\.[^./]+$/.test(lastSegment);
}
function isAllowedStaticAssetPath(pathname) {
...
export async function proxyCore(req, options) {
...
//# sourceMappingURL=proxyCore.js.map