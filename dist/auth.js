...
async function revokeSessionChain(startSessionId, appId) {
  ...
  await transaction(async (client) => {
    ...
    while (current && !seen.has(current)) {
      ...
      const result = await client.query(sql, [now, current, appId]);
...
async function ensureBrowserSessionCsrf(params) {
  ...
  if (!token) {
    ...
    const resolved = await transaction(async (con) => {
      ...
      const s = await con.query(`SELECT csrf_token FROM ${PREFIX}user_sessions WHERE session_id = $1 AND app_id = $2`, [
        sessionId,
        appId,
      ]);
