...
export default ({ dsName, moduleCode, columns, columnOrder, index, template, editable }) => {
    let cols = columns
    ...
    const fields = cols
        ...
        header: (props) => {
          return <HeaderCell {...props} type="${type}" store={store} accessorKey="${key}${index ?? ''}" title="${label}${index ?? ''}" />;
        },
...
import { useClientSession } ${'from'} '@/components/core/session-context';${editable
        ? `
