...
export function ApiTesterProvider({ children }) {
    ...
    useEffect(() => {
        ...
        if ((urlQueryChanged || urlDsChanged || timestampChanged) &&
            urlQueryMode === 'Query' &&
        ...
        if ((urlPostChanged || urlDsChanged || timestampChanged) &&
            urlQueryMode === 'Post' &&
        ...
        const loadDataSources = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    const targetDs = typeof window !== 'undefined' ? new URLSearchParams(window.location.hash.slice(1)).get('ds') : null;
                    if (targetDs) {
