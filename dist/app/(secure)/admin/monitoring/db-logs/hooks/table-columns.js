...
function LogCell(props) {
    ...
    return (_jsx(Cell, { attributeCode: attributeCode, store: store, rowId: row.id, className: cn('min-w-0', !isValueEmpty && 'cursor-pointer'), dataTip: isValueEmpty ? undefined : displayValue, onClick: isValueEmpty ? undefined : handleFilterClick, children: _jsx("span", { className: cn('min-w-0 flex-1 truncate text-sm', isValueEmpty ? 'text-muted-foreground text-xs' : 'text-primary hover:underline'), children: isValueEmpty ? '-' : displayValue }) }));
}
function StatusCell(props) {
    ...
    return (_jsx(Cell, { attributeCode: "level", store: store, rowId: row.id, className: "min-w-0 justify-center", dataTip: config.label, onClick: level == null ? undefined : handleLevelClick, children: _jsxs("div", { className: cn('flex min-w-0 max-w-full cursor-pointer items-center gap-1 rounded-full px-2 py-0', config.bgColor), children: [_jsx(Icon, { className: cn('size-4 shrink-0', config.color) }), _jsx("span", { className: cn('min-w-0 truncate font-medium text-sm', config.color), children: config.label })] }) }));
}
    ...
    return useMemo(() => {
        const columns = [
            ...
            },
            {
                ...
                size: 90,
                header: (props) => {
                    return _jsx(HeaderCell, { ...props, type: "Number", align: "center", store: store, accessorKey: "level", title: "Level" });
                },
