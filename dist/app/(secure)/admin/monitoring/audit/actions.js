 ...
 */
export async function queryAuditStats({ client }) {
 ...
 */
export async function queryAuditFilterOptions({ client, }) {
    ...
    const [datasourcesResult, usersResult, attributesResult, valueTypesResult] = await Promise.all([
        client.query(`SELECT DISTINCT datasource_id FROM ${PREFIX}audit WHERE app_id = $1 ORDER BY datasource_id`, [appId]),
        client.query(`SELECT DISTINCT updated_by FROM ${PREFIX}audit WHERE app_id = $1 ORDER BY updated_by`, [appId]),
        client.query(`SELECT DISTINCT attribute_code FROM ${PREFIX}audit WHERE app_id = $1 ORDER BY attribute_code`, [appId]),
        client.query(`SELECT DISTINCT value_type FROM ${PREFIX}audit WHERE app_id = $1 ORDER BY value_type`, [appId]),
    ]);
    return {
