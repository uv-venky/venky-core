...
export function LookupTypesPanel({ selectedLookupType, onSelectLookupType }) {
  ...
  return _jsxs('div', {
    ...
    children: [
        ...
        children: isLoading
          ...
          : filteredLookupTypes.length === 0
            ? _jsx('div', {
                ...
                children: searchQuery ? 'No lookup types found' : 'No lookup types. Click + to add one.',
              })
            : _jsx('div', {
                className: 'space-y-2',
                children: filteredLookupTypes.map((lookupType) =>
                  _jsx(
                    LookupTypeItem,
                    {
      ...
      _jsx(AddLookupTypeDialog, { open: isAddDialogOpen, onOpenChange: setIsAddDialogOpen, store: store }),
    ],
  });
}
