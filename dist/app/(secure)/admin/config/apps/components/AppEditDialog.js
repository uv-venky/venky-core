...
export function AppEditDialog({ app, store, open, onOpenChange }) {
    ...
    return (_jsx(Dialog, { open: open, onOpenChange: (open) => {
        ...
        }, children: _jsxs(DialogContent, { className: "sm:max-w-[500px]", children: [_jsxs(DialogHeader, { children: [_jsx(DialogTitle, { children: row.appId ? 'Edit App' : 'New App' }), _jsx(DialogDescription, { children: "Configure the app details. The status token is used to authenticate status API requests." })] }), _jsxs("div", { className: "grid gap-4 py-4", children: [_jsx(TextInput, { label: "App ID", value: row.appId || '', onChange: (value) => store.setValue('appId', value || ''), labelOnTop: true, required: true, disabled: isPersisted }), _jsx(TextInput, { label: "Name", value: row.name || '', onChange: (value) => store.setValue('name', value || ''), labelOnTop: true, required: true }), _jsx(TextInput, { label: "Full URL", value: row.fullUrl || '', onChange: (value) => store.setValue('fullUrl', value || ''), labelOnTop: true, required: true, placeholder: "https://example.com" }), _jsx(ComboboxInput, { label: "Icon", labelOnTop: true, value: row.icon || 'MiniLogo', options: iconOptions, getValue: (opt) => opt.value, getLabel: (opt) => opt.label, getIcon: (opt) => _jsx(AppIcon, { icon: opt.value }), onSelect: (value) => store.setValue('icon', (value || 'MiniLogo')), placeholder: "Select an icon...", searchPlaceholder: "Search icons...", emptyText: "No icons found" }), _jsxs("div", { className: "space-y-2", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("span", { className: "font-medium text-sm", children: "Status Token" }), _jsxs(Button, { type: "button", variant: "outline", size: "sm", onClick: handleGenerateToken, className: "h-7", children: [_jsx(RefreshCw, { className: "mr-1 size-3" }), "Generate"] })] }), _jsx(PasswordInput, { value: row.statusToken || '', onChange: (value) => store.setValue('statusToken', value || null), placeholder: "Leave empty or generate a secure token", className: "w-full" }), _jsx("p", { className: "text-muted-foreground text-xs", children: "Bearer token used to authenticate requests to /api/p/status endpoint" })] })] }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "outline", onClick: handleCancel, disabled: isPosting, children: "Cancel" }), _jsx(Button, { onClick: handleSave, disabled: isPosting || !isDirty, children: isPosting ? (_jsxs(_Fragment, { children: [_jsx(Loader2, { className: "mr-2 size-4 animate-spin" }), "Saving..."] })) : (_jsxs(_Fragment, { children: [_jsx(Check, { className: "mr-2 size-4" }), "Save"] })) })] })] }) }));
}
