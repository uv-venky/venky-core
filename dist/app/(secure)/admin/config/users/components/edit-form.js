...
export default function UsersEditForm({ store }) {
  ...
  return _jsxs('div', {
    ...
    children: [
      _jsx(TextInput, { label: 'Display Name', store: store, attributeCode: 'displayName', required: true }),
      _jsx(TextInput, {
        disabled: fromDB,
      ...
      !fromDB &&
        _jsxs(_Fragment, {
          children: [
            ...
            _jsx(BooleanInput, { label: 'Send New User Email', store: store, attributeCode: 'sendNewUserEmail' }),
          ],
        }),
      _jsx(DatePickerField, { label: 'Start Date', store: store, attributeCode: 'startDate', required: true }),
      _jsx(DatePickerField, { label: 'End Date', store: store, attributeCode: 'endDate' }),
      _jsx(TextInput, { label: 'Location Name', store: store, attributeCode: 'locationName' }),
      fromDB && _jsx(BooleanInput, { label: 'Locked', store: store, attributeCode: 'locked' }),
      _jsx(BooleanInput, { label: 'Force Password Change', store: store, attributeCode: 'forcePasswordChange' }),
      !fromDB && _jsx(CopyRolesFromUserField, {}),
    ],
  });
}
