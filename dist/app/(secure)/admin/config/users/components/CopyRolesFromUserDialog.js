...
export function CopyRolesFromUserDialog({ rowId, onClose }) {
    ...
    if (showUserSelection) {
        return (_jsx(LOVDialog, { open: true, onOpenChange: (open) => {
                if (!open) {
                    onClose();
                }
            }, store: userListStore, tableColumns: tableColumns, smartSearchColumns: smartSearchColumns, onSelect: handleUserSelect, title: "Copy Roles from User", width: 1100, height: 650, singleSelection: true }));
    }
    return null;
}
