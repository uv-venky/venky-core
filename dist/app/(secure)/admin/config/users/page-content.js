...
function UsersPageContentInner({ toolbarContent }) {
    ...
    return (_jsx(PageLayoutTemplate, { title: "Users", subTitle: "Manage Users", icon: _jsx(ShoppingCart, { className: "h-12 w-12 text-muted-foreground" }), store: store, smartSearchColumns: smartSearchColumns, tableColumns: tableColumns, pageId: "users-page", itemId: "users", editForm: _jsx(EditForm, { store: store }), getDefaultRow: () => {
            const now = new Date();
            now.setSeconds(0);
        ...
        }, addNewButtonText: "Add New User", popupWidth: 680, popupHeight: 690, disableHeaderFilters: true, toolbarContent: toolbarContent, handleSave: handleSave, showExportButton: true, defaultVisibleColumnOrder: defaultVisibleColumnOrder, enableNaturalLanguageSearch: true }));
}
