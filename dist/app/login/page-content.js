...
function LoginTabs({ tabs, legalNotice }) {
    ...
    return (_jsxs(Tabs, { defaultValue: defaultTab, className: "w-full", children: [_jsx(TabsList, { className: cn('grid w-full rounded-full bg-gray-800/70 p-1', TAB_GRID_CLASS[tabs.length] ?? 'grid-cols-2'), children: tabs.map((tab) => (_jsx(TabsTrigger, { value: tab.id, className: "rounded-full data-[state=active]:bg-primary data-[state=active]:text-white", children: tab.label }, tab.id))) }), tabs.map((tab) => (_jsx(TabsContent, { value: tab.id, className: "pt-6", children: renderLoginTabContent(tab, legalNotice) }, tab.id)))] }));
}
