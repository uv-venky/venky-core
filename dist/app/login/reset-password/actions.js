...
import { createPasswordResetToken, signOutAllUserSessions, validateNewPassword, validatePasswordResetToken, } from '../../../lib/core/server/password-reset';
...
export const requestPasswordReset = withDBAction(async (client, _prev, formData) => {
    const username = formData.get('username')?.trim().toLowerCase() ?? '';
    const email = formData.get('email')?.trim().toLowerCase() ?? '';
    return requestPasswordResetForUserWithClient(client, { username, email });
});
async function requestPasswordResetForUserWithClient(client, input) {
    ...
    const { rows } = await client.query(`SELECT
      ...
      locked,
      password_hash
    FROM ${PREFIX}users WHERE lower(email) = $1 AND user_name = $2`, [email, username]);
    const user = rows[0];
...
async function isValidPasswordResetTokenWithClient(client, encodedToken) {
    let decryptedToken = '';
    try {
...
export async function isValidPasswordResetToken(encodedToken) {
    return await transaction(async (client) => {
        return await isValidPasswordResetTokenWithClient(client, encodedToken);
    });
}
export async function validatePasswordResetTokenValue(encodedToken) {
    return isValidPasswordResetToken(encodedToken);
}
export async function changePasswordWithResetToken(input) {
    ...
    FROM ${PREFIX}users WHERE user_name = $1`, [userName]);
        ...
        await client.query(`UPDATE ${PREFIX}users SET force_password_change = false, password_hash = $1, last_password_reset = now(), last_password_reset_ip_address = $2, last_password_reset_by = $3, previous_password_hashes = $4::jsonb WHERE user_name = $5`, [passwordHash, ipAddress, user.user_name, JSON.stringify(previousPasswordHashes), user.user_name]);
