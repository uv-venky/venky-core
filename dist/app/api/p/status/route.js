...
export const GET = withDBRoute(async function callback(client, req) {
    ...
    const appResult = await client.query(`SELECT app_id, name, full_url FROM ${PREFIX}apps WHERE status_token = $1 AND app_id = $2`, [token, appId]);
    ...
    try {
        const usersResult = await client.query(`SELECT COUNT(DISTINCT us.user_name) as count 
         ...
         WHERE us.app_id = $1 
           ...
           AND us.signed_out_at IS NULL`, [appId]);
