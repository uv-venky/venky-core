...
function ButtonGroupText({ className, asChild = false, ...props }) {
    ...
    return (_jsx(Comp, { className: cn("flex items-center gap-2 rounded-md border bg-muted px-4 font-medium text-sm shadow-xs [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none", className), ...props }));
}
function ButtonGroupSeparator({ className, orientation = 'vertical', ...props }) {
    return (_jsx(Separator, { "data-slot": "button-group-separator", orientation: orientation, className: cn('!m-0 relative self-stretch bg-input data-[orientation=vertical]:h-auto', className), ...props }));
}
