/** biome-ignore-all lint/nursery/useSortedClasses:CoreFile */
'use client';
...
function ChartTooltipContent({ active, payload, className, indicator = 'dot', hideLabel = false, hideIndicator = false, label, labelFormatter, valueFormatter, labelClassName, formatter, color, nameKey, labelKey, }) {
    ...
    return (_jsxs("div", { className: cn('border-border/50 bg-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl', className), children: [!nestLabel ? tooltipLabel : null, _jsx("div", { className: "grid gap-1.5", children: payload.map((item, index) => {
                    ...
                    return (_jsx("div", { className: cn('[&>svg]:text-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5', indicator === 'dot' && 'items-center'), children: formatter && item?.value !== undefined && item.name ? (formatter(item.value, item.name, item, index, item.payload)) : (_jsxs(_Fragment, { children: [itemConfig?.icon ? (_jsx(itemConfig.icon, {})) : (!hideIndicator && (_jsx("div", { className: cn('shrink-0 rounded-[2px] border-(--color-border) bg-(--color-bg)', {
                                    ...
                                    } }))), _jsxs("div", { className: cn('flex flex-1 gap-2 justify-between leading-none', nestLabel ? 'items-end' : 'items-center'), children: [_jsxs("div", { className: "grid gap-1.5", children: [nestLabel ? tooltipLabel : null, _jsx("span", { className: "text-muted-foreground", children: itemConfig?.label || item.payload.name || item.name })] }), item.value !== undefined && item.value !== null && (_jsx("span", { className: "text-foreground font-mono font-medium tabular-nums", children: typeof item.value === 'number' && valueFormatter
                                                ? maskDigits(valueFormatter(item.value))
                                                : item.value != null && maskDigits(String(item.value)) }))] })] })) }, item.dataKey));
                }) })] }));
}
...
function ChartLegendContent({ className, hideIcon = false, payload, verticalAlign = 'bottom', nameKey }) {
    ...
    return (_jsx("div", { className: cn('flex items-center justify-center gap-4 flex-wrap', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className), children: payload.map((item) => {
