...
function ExcelUpload(props) {
        ...
        if (callbackFunction) {
          ...
          if (result.status === 'SUCCESS') {
            if (Array.isArray(result.result)) {
              ...
              throw new UserError(
                `Developer Error: Callback function ${callbackFunction} must return an array of records! Instead it returned ${JSON.stringify(result.result)}`,
              );
            }
  ...
  return _jsx('div', {
    ref: ref,
    className: clsx('flex w-full flex-1 items-center gap-2', className),
    children: _jsxs('div', {
      ...getRootProps(),
      className: clsx(
      ...
      children: [
        _jsx('input', { 'data-testid': 'excel-upload-input', ...getInputProps() }),
        _jsx('div', {
          className: cn('flex w-full items-center justify-center text-center text-green-400 text-sm'),
          children: isDragActive ? 'Drop it now...' : 'Drag and drop an Excel or CSV file here...',
        }),
      ],
    }),
  });
