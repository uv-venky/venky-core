  ...
  if (defaultFunction) {
    ...
    if (result.status === 'SUCCESS') {
      if (Array.isArray(result.result)) {
        ...
        throw new Error(
          `Developer Error: Default function ${defaultFunction} must return an array of records! Instead it returned ${JSON.stringify(result.result)}`,
        );
      }
      ...
      if (val !== undefined && val != null) {
        ...
        if (isDateType(attr.type)) {
          ...
          if (attr.maxLength && val.length > attr.maxLength) {
            throw new Error(
              `Value '${val}' for ${label} at row ${i + 1} is too long ${val.length} chars! Maximum length allowed is ${attr.maxLength} chars!`,
            );
          }
          record[attr.code] = val;
