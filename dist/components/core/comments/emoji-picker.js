...
export function CustomEmojiPicker({ onEmojiSelect, trigger, size = 'default' }) {
  ...
  const defaultTrigger = _jsxs(Button, {
    size: size,
    variant: 'ghost',
    className: `${size === 'sm' ? 'h-7 px-2' : 'h-7 px-2'} text-muted-foreground hover:text-foreground`,
    children: [
      _jsx(Smile, { className: 'mr-1 h-3 w-3' }),
      size !== 'sm' && _jsx('span', { className: 'text-xs', children: 'React' }),
    ],
  });
  return _jsxs(Popover, {
    ...
    children: [
      _jsx(PopoverTrigger, { asChild: true, children: trigger || defaultTrigger }),
      _jsx(PopoverContent, {
