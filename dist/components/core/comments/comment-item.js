...
export function CommentItem({
  ...
  const renderAttachment = (attachment) => {
    ...
    return _jsxs(
      'div',
      {
        className: 'flex max-w-xs items-center gap-2 rounded-lg border bg-background/50 p-2',
        children: [
          _jsxs('div', {
            className: 'min-w-0 flex-1',
            children: [
              _jsx('p', { className: 'truncate font-medium text-sm', children: attachment.name }),
              _jsx('p', { className: 'text-muted-foreground text-xs', children: formatFileSize(attachment.size) }),
            ],
          }),
          _jsx(Button, {
            size: 'sm',
  ...
  return _jsx('div', {
    ...
    children: _jsxs('div', {
      className: `flex gap-3 p-4 transition-colors hover:bg-muted/30 ${isOwnComment ? 'flex-row-reverse' : ''}`,
      children: [
        _jsx(UserAvatar, { userName: comment.author, showDisplayName: false, showEmail: false }),
        _jsxs('div', {
          className: `min-w-0 max-w-[80%] flex-1 ${isOwnComment ? 'items-end' : ''}`,
          children: [
            _jsxs('div', {
              className: `mb-1 flex items-center gap-2 ${isOwnComment ? 'flex-row-reverse' : ''}`,
              children: [
                _jsx('span', { className: 'font-medium text-sm', children: isOwnComment ? 'You' : comment.author }),
                comment.author &&
                  _jsx(Badge, { variant: 'secondary', className: 'px-1.5 py-0.5 text-xs', children: comment.author }),
                _jsx('span', {
                  className: 'text-muted-foreground text-xs',
                  children: formatDistanceToNow(comment.createdAt, { addSuffix: true }),
                }),
                comment.updatedAt && _jsx('span', { className: 'text-muted-foreground text-xs', children: '(edited)' }),
              ],
            }),
            comment.parentId && _jsx(MessageReference, { comment: comment, onNavigateToMessage: onNavigateToMessage }),
            _jsxs('div', {
              className: cn(
              ...
              children: [
                _jsx('div', { className: 'whitespace-pre-wrap break-all text-sm', children: comment.comment }),
                comment.attachments &&
                  comment.attachments.length > 0 &&
                  _jsx('div', {
                    className: 'mt-2 flex flex-wrap gap-2',
                    children: comment.attachments.map(renderAttachment),
                  }),
              ],
            }),
            _jsxs('div', {
              className: `mt-2 flex flex-wrap items-center gap-1 ${isOwnComment ? 'flex-row-reverse' : ''}`,
              children: [
                enableLike &&
                  _jsxs(Button, {
                    size: 'sm',
                    ...
                    'data-tip':
                      totalLikes > 0
                        ...
                        : 'Click to like',
                    children: [
                      _jsx(Heart, { className: `mr-1 h-3 w-3 ${isLiked ? 'fill-current' : ''}` }),
                      totalLikes > 0 && _jsx('span', { className: 'text-xs', children: totalLikes }),
                    ],
                  }),
                enableEmojiReactions &&
                  _jsx(CustomEmojiPicker, {
                ...
                enableEmojiReactions &&
                  ...
                  _jsxs(_Fragment, {
                    children: [
                      _jsx('div', { className: 'mx-1 h-4 w-px bg-border' }),
                      isReacting && _jsx(Loader2, { className: 'h-4 w-4 animate-spin' }),
                        ...
                        .map(([emoji, users]) => {
                          ...
                          return _jsxs(
                            Button,
                            {
                              ...
                              children: [
                                _jsx('span', { className: 'text-sm', children: emoji }),
                                _jsx('span', { children: users.length }),
                              ],
                            },
                            emoji,
                          );
                ...
                _jsxs(Button, {
                  ...
                  children: [_jsx(Reply, { className: 'mr-1 h-3 w-3' }), 'Reply'],
                }),
                isOwnComment &&
                  onEdit &&
                  _jsxs(DropdownMenu, {
                    children: [
                      _jsx(DropdownMenuTrigger, {
                        ...
                        children: _jsx(Button, {
                          ...
                          children: _jsx(MoreHorizontal, { className: 'h-3 w-3' }),
                        }),
                      }),
