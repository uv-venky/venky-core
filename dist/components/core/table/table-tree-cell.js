...
function TableTreeCell({ attributeCode, labelAttributeCode, className, row, onClick, endComponent, feedbackMask }) {
  ...
  function renderSwitcher() {
    ...
    return _jsx('span', {
      ...
      children: _jsx('span', {
        className:
          'tree-expander-icon z-10 bg-background text-muted-foreground group-hover/row:bg-background group-data-[state=selected]/row:bg-background',
        children: isExpanding
          ? _jsx(Loader2, { className: 'animate-spin' })
          : isExpanded
            ? _jsx(MinusCircle, { className: '' })
  ...
  return _jsxs(Cell, {
    ...
    feedbackMask: feedbackMask,
    children: [
      _jsx(Indent, { level: level, isStart: isStart, isEnd: isEnd }),
      renderSwitcher(),
      _jsx('span', {
        role: 'button',
        tabIndex: 0,
        onClick: onClick,
        className: 'ml-2 flex h-full items-center truncate',
        children: label,
      }),
      endComponent,
    ],
