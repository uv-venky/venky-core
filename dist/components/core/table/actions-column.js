...
export function ActionsColumnCell({ actions, rowId, className, table, store }) {
  ...
  return _jsxs(_Fragment, {
    children: [
      _jsxs('div', {
        className: cn('flex items-center justify-center gap-1 px-2', className),
        children: [
          iconActions.map((action) => {
            if (isTableActionWithRender(action)) {
              return _jsx(
                Fragment,
                { children: action.render({ rowId, table, asIconButton: true, store }) },
                'action-render-icon',
              );
            }
            const isDisabled =
          ...
          dropdownActions.length > 0 &&
            _jsxs(DropdownMenu, {
              onOpenChange: (open) => open && store.setCurrentRowId(rowId),
              children: [
                _jsx(DropdownMenuTrigger, {
                  ...
                  children: _jsx(Button, {
                    ...
                    children: _jsx(MoreHorizontal, { className: 'size-4' }),
                  }),
                }),
                _jsx(DropdownMenuContent, {
                  ...
                  children: dropdownActions.map((action, index) => {
                    if (isTableActionWithRender(action)) {
                      return _jsx(
                        Fragment,
                        { children: action.render({ rowId, table, asIconButton: false, store }) },
                        'action-render-dropdown',
                      );
                    }
                    const isDisabled =
                      typeof action.disabled === 'function' ? action.disabled(rowId) : (action.disabled ?? false);
                    ...
                    return _jsxs(
                      ...
                      {
                        children: [
                          showSeparator && _jsx(DropdownMenuSeparator, {}),
                          _jsxs(DropdownMenuItem, {
                            ...
                            children: [
                              _jsx('span', {
                                className: cn(
                                  'flex items-center',
                                  action.variant === 'destructive' && '[&_svg]:text-red-500!',
                                ),
                                children: action.icon,
                              }),
                              _jsx('span', {
