...
function HeaderFilterInput({ column, store }) {
  ...
  return _jsxs('div', {
    className: 'm-1 flex items-center gap-1 rounded-sm border bg-background',
    children: [
      _jsx(DropdownMenuField, {
        ...
        children: _jsx(Icon, { 'data-tip': operator, className: 'size-3.5 shrink-0' }),
      }),
      _jsxs('div', {
        ...
        children: [
          ...
          _jsxs('div', {
            ...
            children: [
              ...
              isDirty &&
                ((Array.isArray(value) ? value.length > 0 : !isEmpty(value)) || !hasEditor(operator)) &&
                _jsx(Button, {
                  variant: 'ghost',
                  size: 'icon',
...
export default function TableHeaderFilters({ table, store, columns }) {
  ...
  return _jsx(TableRow, {
    ...
    children: leafColumns.map((col) => {
      const c = columns.find((s) => s.key === col.id);
      return _jsx(
        TableHead,
        {
          style: { width: `var(--col-${col.id}-size)` },
          className: 'p-0',
          children: c ? _jsx(HeaderFilterInput, { column: c, store: store }) : null,
        },
        col.id,
      );
    }),
