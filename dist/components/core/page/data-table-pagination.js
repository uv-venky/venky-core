...
function PaginationButton({ onClick, disabled, label, icon, testId }) {
  ...
  return _jsx(Button, {
    ...
    'data-tip': label,
    'data-testid': testId,
    children: busy ? _jsx(Loader2, { className: 'size-4 animate-spin', 'aria-hidden': 'true' }) : icon,
  });
}
function DataTablePagination({ table, pageSizeOptions = PAGE_SIZE_OPTIONS, store, hideRowsPerPageSelector = false }) {
  ...
  return _jsx('div', {
    className: 'flex flex-col-reverse items-center justify-between gap-4 overflow-auto p-1 sm:flex-row sm:gap-8',
    children: _jsxs('div', {
      className: 'flex select-none flex-col-reverse items-center gap-4 sm:flex-row sm:gap-6 lg:gap-8',
      children: [
            ...
            children: [
              !hideRowsPerPageSelector &&
                _jsxs('div', {
                  ...
                  children: [
                    _jsx('p', { className: 'whitespace-nowrap font-medium text-sm', children: 'Rows per page' }),
                    _jsxs(Select, {
                      ...
                      children: [
                        _jsx(SelectTrigger, {
                          ...
                          children: busy
                            ? _jsx(Loader2, { className: 'size-4 animate-spin', 'aria-hidden': 'true' })
                            : _jsx(SelectValue, { placeholder: table.getState().pagination.pageSize }),
                        }),
                        _jsx(SelectContent, {
                          side: 'top',
        ...
        _jsxs('div', {
          ...
          children: [
            _jsx(PaginationButton, {
              ...
              icon: _jsx(ChevronsLeft, { className: 'size-4', 'aria-hidden': 'true' }),
            ...
            _jsx(PaginationButton, {
              ...
              disabled: busy || !table.getCanPreviousPage(),
              icon: _jsx(ChevronLeft, { className: 'size-4', 'aria-hidden': 'true' }),
              testId: 'pagination-previous-page',
            }),
            _jsx(PaginationButton, {
              label: 'Go to next page',
              icon: _jsx(ChevronRight, { className: 'size-4', 'aria-hidden': 'true' }),
              onClick: async () => {
                table.nextPage();
                if (store) {
                  await store.goToPage(pageIndex + 1);
            ...
            _jsx(PaginationButton, {
              ...
              disabled: busy || !table.getCanNextPage(),
              icon: _jsx(ChevronsRight, { className: 'size-4', 'aria-hidden': 'true' }),
