...
export class StoreClass {
  ...
  _getPrimaryKeySync = (pkAttributes, r) => {
    ...
    const pk = pkAttributes.reduce(
      (key, attr) => `${key}${key === '' ? '' : '-'}${r[attr.code] ?? this.newRecordID()}`,
      '',
    );
    if (!pk) {
  ...
  async executeQuery(
    { query = {}, noClear, force = false, handleResponse, refreshOrPagination = false } = { query: {} },
  ) {
...
export function useStore(props) {
  ...
  useEffect(() => {
    ...
    if (store.autoQuery && !store.initialQueryFired() && store.status() === 'ok' && !isWaitingForSavedSearches(store)) {
      store.executeQuery();
    }
