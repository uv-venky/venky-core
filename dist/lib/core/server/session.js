...
export async function getUserSession() {
    ...
    WHERE u.user_name = $1`, [userName, session.id, appId]);
...
export async function makeBackgroundJobSession(client, userName, jobId) {
    const { rows: userRows } = await client.query(`SELECT user_name, user_id, email, display_name, settings FROM ${PREFIX}users WHERE user_name = $1`, [userName]);
    ...
    const { rows: roleRows } = await client.query(`SELECT r.role_code FROM ${PREFIX}user_roles ur, ${PREFIX}roles r 
