...
export async function assertCsrf(client, session, req) {
    ...
    if (isCsrfExempt(url.pathname, req.method))
        return;
    ...
    const result = await client.query(`SELECT csrf_token FROM ${PREFIX}user_sessions WHERE session_id = $1 LIMIT 1`, [session.id]);
    let stored = result.rows[0]?.csrf_token;
    if (!stored) {
        const minted = randomBytes(32).toString('hex');
        const upd = await client.query(`UPDATE ${PREFIX}user_sessions SET csrf_token = $1 WHERE session_id = $2 AND csrf_token IS NULL RETURNING csrf_token`, [minted, session.id]);
        if (upd.rows[0]?.csrf_token) {
            stored = upd.rows[0].csrf_token;
        }
        else {
            const reread = await client.query(`SELECT csrf_token FROM ${PREFIX}user_sessions WHERE session_id = $1 LIMIT 1`, [session.id]);
