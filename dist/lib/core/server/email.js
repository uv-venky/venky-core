...
async function resolveQueuedMailOptions(options) {
...
export async function sendEmailNow(allOptions) {
    let options = await resolveQueuedMailOptions(allOptions);
...
let timeout = null;
export async function sendEmail(options, client) {
    try {
        ...
        await client.query(`INSERT INTO ${PREFIX}email_requests (to_address, subject, mail_options, app_id, scheduler_id) VALUES ($1, $2, $3, $4, $5)`, [to, subject, JSON.stringify(options), appId, schedulerId]);
        if (timeout) {
...
export async function sendEmailWithTemplate({ template, context, to, cc, subject, attachments, client, }) {
    await sendEmail({
        to,
        cc,
        subject,
 ...
 */
export async function sendTestEmailWithSession(client, session) {
...
export async function sendNewUserEmail({ email, userName, password, accountId, isInternal, client, }) {
...
export async function bulkQueueNewUserEmails({ users, isInternal, client, }) {
    if (users.length === 0)
        return;
    const basePath = process.cwd();
    let subject = DEFAULT_NEW_USER_SUBJECT;
    ...
    try {
        await client.query(`INSERT INTO ${PREFIX}email_requests (to_address, subject, mail_options, app_id, scheduler_id) VALUES ${valueClauses.join(', ')}`, params);
        if (timeout) {
            clearTimeout(timeout);
        }
        timeout = setTimeout(() => {
...
export async function sendPasswordResetEmail({ email, token, key, userName, client, }) {
    const basePath = process.cwd();
