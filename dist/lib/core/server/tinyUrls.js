...
export async function createTinyUrl({ client, userName, url, expiresAt, isPublic = false, }) {
    ...
    await client.query(`INSERT INTO ${PREFIX}tiny_urls (short_id, original_url, user_name, app_id, expires_at) VALUES ($1, $2, $3, $4, $5)`, [shortId, url, userName, appId, expiresAt]);
...
export async function getOriginalUrl({ client, shortId }) {
    ...
    const result = await client.query(`SELECT original_url FROM ${PREFIX}tiny_urls WHERE short_id = $1 AND (expires_at IS NULL OR expires_at > NOW()) AND app_id = $2`, [shortId, appId]);
    if (result.rows.length === 0) {
        return '/404';
    }
    const originalUrl = result.rows[0].original_url;
    await client.query(`UPDATE ${PREFIX}tiny_urls SET click_count = click_count + 1 WHERE short_id = $1 AND app_id = $2`, [shortId, appId]);
    return originalUrl;
}
