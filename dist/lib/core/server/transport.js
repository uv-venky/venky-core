    ...
    const flushBuffer = async () => {
        ...
        for (const batch of batches) {
            ...
            batch.forEach((log, i) => {
                ...
                values.push(log.level, log.msg.slice(0, 1000), log.sessionId ?? null, log.apiName?.slice(0, 128) ?? null, log.trackId ?? null, log.dataSource?.slice(0, 40) ?? null, log.userName ?? null, log.hostname, log.pid, new Date(log.time), log.appVersion ?? appVersion, appId);
            });
            const sql = `
