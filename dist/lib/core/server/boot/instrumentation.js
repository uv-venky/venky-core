...
function resolvePingOptions(options) {
    ...
    return {
        firstDelayMs: options?.firstDelayMs ?? Number(process.env.VENKY_INSTRUMENTATION_PING_FIRST_DELAY_MS ?? (isDev ? 3000 : 1000)),
        maxRetries: options?.maxRetries ?? Number(process.env.VENKY_INSTRUMENTATION_PING_RETRIES ?? (isDev ? 10 : 5)),
        retryDelayMs: options?.retryDelayMs ?? Number(process.env.VENKY_INSTRUMENTATION_PING_RETRY_DELAY_MS ?? (isDev ? 2000 : 1000)),
        fetchTimeoutMs: options?.fetchTimeoutMs ?? Number(process.env.VENKY_INSTRUMENTATION_PING_TIMEOUT_MS ?? (isDev ? 30_000 : 5000)),
    };
}
        ...
        for (let attempt = 1; attempt <= ping.maxRetries; attempt++) {
            try {
                ...
                if (attempt === ping.maxRetries) {
                    console.warn('Instrumentation: Failed to invoke /api/ping after all retries (this is OK, initialization will happen on first request)', {
