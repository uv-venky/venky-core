...
function buildPoolSsl() {
    ...
    if (PG_SSL_CA || PG_SSL_CERT || PG_SSL_KEY) {
        ...
        if ((PG_SSL_CERT && !PG_SSL_KEY) || (!PG_SSL_CERT && PG_SSL_KEY)) {
            logger.warn('DB TLS client certificate configuration is incomplete; both PG_SSL_CERT and PG_SSL_KEY are required');
        }
    }
    else {
...
export function getReadOnlyPool() {
    ...
    if (!readonlyDbUrl && process.env.NODE_ENV === 'production' && process.env.VENKY_ALLOW_RO_FALLBACK !== '1') {
        throw new Error('readonlyDbUrl is required in production. Provision a separate read-only Postgres role and set db.readonlyDbUrl in config (or set VENKY_ALLOW_RO_FALLBACK=1 to acknowledge the fallback).');
    }
...
export async function parallelReadQueries(...queries) {
    ...
    try {
        await Promise.all(queries.map(async (_, i) => {
