...
export async function putTTLValue(client, key, value, ttlSeconds) {
    ...
    await client.query(`INSERT INTO ${PREFIX}ttl_store(key, data, expires_at, app_id)
           VALUES ($1, $2::jsonb, now() + ($3::int) * interval '1 second', $4)
           ON CONFLICT (key)
           DO UPDATE SET data = EXCLUDED.data,
                         expires_at = EXCLUDED.expires_at,
                         created_at = statement_timestamp()`, [key, JSON.stringify(value), ttlSeconds, appId]);
}
export async function getTTLValue(client, key) {
    const appId = getConfig('getTTLValue').appId;
    const { rows } = await client.query(`SELECT data FROM ${PREFIX}ttl_store WHERE key = $1 AND expires_at > now() AND app_id = $2`, [key, appId]);
    return rows[0]?.data ?? null;
}
export async function consumeTTLValue(client, key) {
    const appId = getConfig('consumeTTLValue').appId;
    const { rows } = await client.query(`DELETE FROM ${PREFIX}ttl_store
      WHERE key = $1
        AND expires_at > now()
        AND app_id = $2
      RETURNING data`, [key, appId]);
    return rows[0]?.data ?? null;
}
