...
function ignoreMigrationChecksumMismatchInDev() {
    return process.env.NODE_ENV === 'development' && isEnvTruthy(process.env.VENKY_IGNORE_MIGRATION_CHECKSUM_MISMATCH);
}
const directory = path.join(process.cwd(), 'migrations');
export async function runMigrations(client, logger) {
    const filePaths = (await fs.promises.readdir(directory)).filter((file) => !file.startsWith('.'));
    filePaths.sort();
    const installed_scripts = await client.query(`SELECT version, name, checksum FROM ${PREFIX}migrations ORDER BY version ASC`);
    ...
    for (const file of filePaths) {
        ...
        const checksum = generateChecksum(content);
        await client.query(`INSERT INTO ${PREFIX}migrations(version, name, success, checksum, execution_time)
          VALUES($1, $2, $3, $4, $5)`, [version, name, true, checksum, duration]);
    }
}
