...
async function flushBuffer() {
    if (global._$activityBuffer.length === 0)
        return;
    ...
    batch.forEach((activity, i) => {
        ...
        values.push(activity.userName, truncateString(activity.eventType, 40), truncateString(activity.eventId, 128), meta, truncateString(activity.pageUrl, 128), truncateString(activity.dataSource, 40), activity.elapsedTimeMs ?? null, activity.sessionId ?? null, activity.rowCount ?? null, activity.apiName ?? null, activity.trackId ?? logger.context.getStore()?.trackId ?? null, activity.appVersion ?? APP_VERSION, appId, activity.domainId ?? null);
    });
    const sql = `
...
export async function shutdownActivityBuffer() {
    ...
    if (global._$activityFlushInterval) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Activity flush timeout')), 3000));
...
function looksLikeStringifiedValue(s) {
    if (s.length < MAX_EVENT_ID_JSON_LIKE)
        return false;
    const trimmed = s.trim();
    if (trimmed.startsWith('{') || trimmed.startsWith('['))
        return true;
    if (trimmed.includes('"_events"') || trimmed.includes('"_eventsCount"'))
        return true;
    return false;
}
 ...
 */
export async function logAccessDenied({ userName, roles, sessionId, domainId, resourceType, resource, reason, }) {
    try {
        await logActivity({
            userName,
