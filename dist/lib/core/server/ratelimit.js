...
export class RateLimiter {
    ...
    refillRate;
    name;
    constructor(name, maxRequests = 1000, rateLimitWindowInMs = 60000 /* 1 minute in milliseconds */) {
        this.name = name;
    ...
    logRejectRateLimitedRequest(ipOrUsername, bucket, requestUrl) {
        if (bucket) {
            bucket.count += 1;
            if (bucket.count === 1) {
...
${alertFooter()}`);
    ...
    rejectRateLimitedRequest(ipOrUsername, requestUrl) {
    ...
    rejectRateLimitedAction(ipOrUsername, requestUrl) {
        const bucket = ipOrUsername ? this.buckets.get(ipOrUsername) : undefined;
        this.logRejectRateLimitedRequest(ipOrUsername, bucket, requestUrl);
        return {
            status: 'ERROR',
...
export function getRateLimiters(name) {
    ...
    const rateLimiters = {
        ...
        unauthenticatedPageAPIRateLimiter: new RateLimiter('Unauthenticated Page API', Number(process.env.UNAUTH_PAGE_API_RATE_LIMIT ?? '50'), 300000),
        unauthenticatedStaticRateLimiter: new RateLimiter('Unauthenticated Static', Number(process.env.UNAUTH_STATIC_RATE_LIMIT ?? '200'), 300000),
        passwordResetRateLimiter: new RateLimiter('Password Reset', Number(process.env.PASSWORD_RESET_RATE_LIMIT ?? '3'), 3600000),
    };
    globalThis._rateLimiters = rateLimiters;
    return rateLimiters;
}
export function cleanupAllRateLimiterData() {
    try {
        const { rateLimiter, unauthenticatedPageAPIRateLimiter, unauthenticatedStaticRateLimiter, passwordResetRateLimiter, } = getRateLimiters('cleanupAllRateLimiterData');
