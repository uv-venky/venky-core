...
export async function cleanOldLogs() {
  ...
  await transaction(async (client) => {
    const logResult = await client.query(
      `DELETE FROM ${PREFIX}logs WHERE created_at < now() - interval '14 days' AND app_id = $1`,
      [appId],
    );
    ...
    const nodeResult = await client.query(
      `DELETE FROM ${PREFIX}scheduler_nodes WHERE last_seen_at < now() - interval '24 hours' AND app_id = $1`,
      [appId],
    );
