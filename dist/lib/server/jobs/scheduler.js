...
async function executeJobRun({ client, job, appId, node, logContext, trackHeartbeat = false, }) {
    const { rows: runRows } = await client.query(`INSERT INTO ${PREFIX}job_history(job_name, node, started_at, app_id) VALUES ($1, $2, statement_timestamp(), $3) RETURNING job_run_id`, [job.name, node, appId]);
    ...
    await client.query(`UPDATE ${PREFIX}job_history SET finished_at = statement_timestamp(), success = $2, error = $3 WHERE job_run_id = $1 AND app_id = $4`, [runId, success, errMsg, appId]);
...
async function scheduleJob(job) {
    ...
    const scheduleNext = async () => {
        await transaction(async (client) => {
            ...
            const res = await client.query(`SELECT next_run FROM ${PREFIX}job_schedule WHERE job_name = $1 AND app_id = $2 AND scheduler_id = $3`, [job.name, appId, schedulerId]);
            let next;
            if (res.rowCount === 0) {
                next = computeNextRun(job.schedule);
                await client.query(`INSERT INTO ${PREFIX}job_schedule(job_name, schedule, next_run, app_id, scheduler_id) VALUES ($1, $2, $3, $4, $5)`, [job.name, job.schedule, next, appId, schedulerId]);
            }
            else {
    ...
    const runJob = async () => {
        const nextRun = await withAdvisoryLock(lockKey, async (client) => {
            ...
            const { appId, schedulerId } = config;
            const { rows } = await client.query(`SELECT next_run FROM ${PREFIX}job_schedule WHERE job_name = $1 AND app_id = $2 AND scheduler_id = $3 FOR UPDATE`, [job.name, appId, schedulerId]);
            ...
            await client.query(`UPDATE ${PREFIX}job_schedule SET next_run = $2, schedule = $3, last_run = statement_timestamp(), updated_at = statement_timestamp() WHERE job_name = $1 AND app_id = $4 AND scheduler_id = $5`, [job.name, due, job.schedule, appId, schedulerId]);
            return due;
        });
        if (nextRun) {
            await scheduleNext();
