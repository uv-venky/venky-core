 ...
 */
function mapTsTypeToParamType(checker, type) {
    if (type.isUnion()) {
        const nonNullableTypes = type.types.filter((member) => !(member.getFlags() & ts.TypeFlags.Undefined) && !(member.getFlags() & ts.TypeFlags.Null));
...
function isParamOptional(paramDecl, paramType) {
    if (paramDecl) {
...
function isParamNullable(paramType) {
...
function collectParamSchemas(checker, actionsType, actionsNode) {
    const result = {};
...
function findActionsDeclaration(sourceFile) {
...
function findActionNameTypeAlias(sourceFile) {
    for (const stmt of sourceFile.statements) {
...
function collectTypeStringLiterals(type, names) {
    if (type.isUnion()) {
...
function collectActionNames(checker, typeAlias) {
    const names = new Set();
...
function generateOutput(schemas) {
    ...
    for (const actionName of Object.keys(schemas).sort()) {
        ...
        for (const p of params) {
            ...
            lines.push(`    { name: ${JSON.stringify(p.name)}, label: ${JSON.stringify(p.label)}, type: ${JSON.stringify(p.type)}${suffix} },`);
        }
        lines.push('  ],');
    }
    lines.push('};');
 ...
 */
function formatWithBiome(filePath, projectRoot, logger) {
    let biomeBin;
    try {
        ...
        const biomePkg = JSON.parse(fs.readFileSync(biomePkgPath, 'utf8'));
        const binRel = typeof biomePkg.bin === 'string' ? biomePkg.bin : biomePkg.bin?.biome;
        if (!binRel)
            throw new Error('no "biome" bin entry in @biomejs/biome');
        biomeBin = path.join(path.dirname(biomePkgPath), binRel);
    }
    catch (err) {
        const reason = err instanceof Error ? err.message : String(err);
        logger.warn(`Skipped biome formatting: @biomejs/biome not resolvable from ${projectRoot} (${reason}). ` +
 ...
 */
export async function generateActionParamSchemas(options = {}) {
...
//# sourceMappingURL=action-param-schemas.js.map